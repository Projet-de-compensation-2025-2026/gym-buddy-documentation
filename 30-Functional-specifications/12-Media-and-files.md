...
| FS-MED-02 | Upload is a two-step: metadata + signed PUT to object storage. |
...
| FS-MED-07 | Download uses a **short-lived signed URL** (default 60 s), issued after an authorization check. |
...
- The implemented upload limit is **8 MiB**; mockup 15 shows an earlier 2 MB limit.
...
- Persistent avatar bytes belong in private object storage; processing uses a bounded temporary directory.
