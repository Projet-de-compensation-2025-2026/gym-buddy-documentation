...
- Joaquim cancelled the TypeScript 6→7 migration (ticket #24 cancelled/closed). Approved and today is TypeScript **6** (`~6.0.2`). Angular 22 `@angular/compiler-cli` **22.1.2** peers `>=6.0 <6.1`; `typescript@7.0.0` is 404 and **7.0.2** cannot land (`ts.readConfigFile is not a function`). That is why the slice was dropped, not a deferred target.
