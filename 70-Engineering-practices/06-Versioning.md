...
2. **PostgreSQL and the Java service run on the existing OVH VPS** (`vps-c39cdf03.vps.ovh.net`). **Done** (ticket #20 **Done**; [gym-buddy-service#7](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/7) / `a07e21e`; Kernel rebuilt as develop `e2ef2aa`). Loopback `healthz` / `readyz` 200 on `127.0.0.1:8080`. PostgreSQL 18.6 / Redis / MinIO on `gym-buddy-data`; `5432` / `6379` / `9000` / `9001` unpublished. **Not** a GHCR / Release tag. Do not claim login on the VPS. Caddy → `127.0.0.1:8080` is the intended public story; Caddy is **not** proven.
