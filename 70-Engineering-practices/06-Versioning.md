...
2. **PostgreSQL and the Java service run on the existing OVH VPS** (`vps-c39cdf03.vps.ovh.net`). **Done** (ticket **#20** **Done / closed**). The VPS container is `gym-buddy-service` `develop` **`e2ef2aa`** (service #5; not a pre-auth snapshot). Loopback `GET /actuator/health/healthz` and `GET /actuator/health/readyz` on `127.0.0.1:8080` both **200**. API bind `127.0.0.1`; data-plane ports `5432` / `6379` / `9000` / `9001` unpublished. Empty/invalid `POST /auth/register` → **422 `VALIDATION`** (route live; **not** a completed register/login). `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull, a Release tag, or a successful replace-from-registry. Caddy → `127.0.0.1:8080` is the intended public story; Caddy is **not** proven.
