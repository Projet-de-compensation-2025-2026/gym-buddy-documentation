...
2. **PostgreSQL and the Java service run on the existing OVH VPS** (`vps-c39cdf03.vps.ovh.net`). **Done** (ticket **#20** **Done / closed**). The VPS Java container on the host is `gym-buddy-service` `develop` **`aea1c56`** (not still `e2ef2aa`; CORS proof used that SHA). Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` on `127.0.0.1:8080` both **200**. API bind `127.0.0.1`; data-plane ports `5432` / `6379` / `9000` / `9001` unpublished. A bad loopback `POST /api/v1/auth/register` → **422 `VALIDATION`** (auth routes exist). `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull, a Release tag, or a successful replace-from-registry. Caddy → `127.0.0.1:8080` is the public story and is **proven from the operator network** (healthz **200**, register **201**, login **200** + JWT). Caddy is **not** proven from the GitHub Pages origin. Login-from-Pages is **not** done.
