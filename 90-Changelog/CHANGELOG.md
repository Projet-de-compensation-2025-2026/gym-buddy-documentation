...
- `gym-buddy-ui` GitHub Pages **live**. Ticket **#30** is **Done**. First UI tag is **v0.1.0**. Sentinel confirmed https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/ returns **HTTP 200** and serves the Angular app with production `baseHref` `/gym-buddy-ui/`. Root 200 is the acceptance. Direct `/register` returns **HTTP 404** with the SPA index body (`404.html`). That is Pages’ static fallback, **not** a broken app and **not** a working auth route. Ticket **#31** (production `apiBaseUrl` / CORS / login-from-Pages) stays **Not Ready**. Do **not** claim the Pages app talks to the VPS. Do **not** claim login-from-Pages. Approved toolchain stays TypeScript **`~6.0.2`** + **pnpm**.
