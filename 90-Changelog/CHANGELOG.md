...
- VPS apply **done** (ticket #20 **Done**; [gym-buddy-service#7](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/7) / `a07e21e`): Java + PostgreSQL 18.6 / Redis / MinIO are up. `GET /api/v1/healthz` and `GET /api/v1/readyz` 200 on `127.0.0.1:8080`. API bind `127.0.0.1`; data-plane ports `5432` / `6379` / `9000` / `9001` unpublished. Running image is **`gym-buddy-service:local`**, **not** a GHCR / Release. Caddy was **not** reachable from Sentinel — do not claim Caddy is proven. This is **not** a login proof. Kernel is rebuilding the VPS from develop for `e2ef2aa` auth. Do **not** claim login is on the VPS. Service auth remains on **`develop` `e2ef2aa`** (service #5). Ticket **#12** is closed.
