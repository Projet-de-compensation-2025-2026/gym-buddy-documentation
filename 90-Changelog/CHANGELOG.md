...
- UI OpenAPI client **landed** on `gym-buddy-ui` `develop` **`b8da6bf`** ([gym-buddy-ui#10](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-ui/pull/10) / ticket **#42** Done). Sentinel confirmed. Branch `feature/42-openapi-client` is gone. Build generates the TypeScript client/types with **orval 8.22.0** from `gym-buddy-openapi@7fa5108` `openapi/bundled.yaml`. **No** `openapi.yaml` (or any YAML copy) in the UI tree. `AuthApi` is a thin wrapper; login / refresh / logout keep `withCredentials`. App version stays **0.1.0**. TypeScript stays **`~6.0.2`**. Ticket **#41** is still in flight (service still handwritten Java DTOs / no openapi-generator). Do **not** claim service codegen landed. Ticket **#40** is **Done**. Ticket **#37** stays **Not Ready**. Do **not** Todo **#37**. Do **not** claim login-from-Pages. Ticket **#24** stays cancelled. Documentation `0.3.0` is **not** bumped. Application repos stay `0.1.x`. Do **not** reopen #36 / #38 / #39 / #43 / #44.
...
- OpenAPI today/target honesty ([08-OpenAPI-contract.md](../40-Technical-specifications/08-OpenAPI-contract.md)): `gym-buddy-openapi` is the source of truth for routes, operations, request/response contracts, and entities. Service and UI **consume** it; they do **not** re-implement the contract. **Today** `gym-buddy-service` still has handwritten Java DTOs and controllers (**no** `openapi-generator` at build) and `gym-buddy-ui` still has handwritten `src/app/api/models.ts` + `auth-api.service.ts` (**no** orval / openapi-typescript at build). Do **not** treat Spring `springdoc` `/v3/api-docs` as the source of truth. Do **not** vendor `openapi.yaml` into `gym-buddy-ui`. **Today** the OpenAPI repo **is** a `$ref` tree plus a checked-in consumer bundle `openapi/bundled.yaml` (openapi #5 / ticket **#40** Done; `develop` **`7fa5108`**). Editors edit the `$ref` tree. Consumers generate from **`openapi/bundled.yaml`**. Codegen in service / UI is tickets **#41** / **#42** — **not** landed. Do **not** claim codegen already landed. Documentation `0.3.0` is **not** bumped. Application repos stay `0.1.x`. Ticket **#24** stays cancelled. Ticket **#37** stays **Not Ready**. Do **not** Todo **#37**.
- OpenAPI split **landed** on `gym-buddy-openapi` `develop` **`7fa5108`** ([gym-buddy-openapi#5](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-openapi/pull/5) / ticket **#40** Done). Sentinel confirmed. Branch `feature/40-openapi-split` is gone. OpenAPI 3.1, `info.version` `0.1.0`. Same six routes / operationIds as before (`getHealthz`, `getReadyz`, `postAuthRegister`, `postAuthLogin`, `postAuthRefresh`, `postAuthLogout`). Checked-in consumer bundle is **`openapi/bundled.yaml`**. Editors edit the `$ref` tree, not the bundle as source. Spring `springdoc` `/v3/api-docs` is **not** the source of truth. Do **not** vendor `openapi.yaml` into `gym-buddy-ui`. Refresh cookie stays `SameSite=Lax`. No codegen in service or UI yet (tickets **#41** / **#42**). Ticket **#37** stays **Not Ready**. Ticket **#24** stays cancelled. Documentation `0.3.0` is **not** bumped. Application repos stay `0.1.x`. Do **not** reopen #36 / #38 / #39 / #43.
