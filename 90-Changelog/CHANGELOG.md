...
- OpenAPI today/target honesty ([08-OpenAPI-contract.md](../40-Technical-specifications/08-OpenAPI-contract.md)): `gym-buddy-openapi` is the source of truth for routes, operations, request/response contracts, and entities. Service and UI **consume** it; they do **not** re-implement the contract. **Today** `gym-buddy-service` still has handwritten Java DTOs and controllers (**no** `openapi-generator` at build) and `gym-buddy-ui` still has handwritten `src/app/api/models.ts` + `auth-api.service.ts` (**no** orval / openapi-typescript at build). Do **not** treat Spring `springdoc` `/v3/api-docs` as the source of truth. Do **not** vendor `openapi.yaml` into `gym-buddy-ui`. **Target:** OpenAPI repo **MAY** split `$ref` files and **MUST** publish one bundled document; service generates models + API interfaces (`pom.xml` is the consumer); UI generates the TS client from that bundle. Do **not** claim codegen already landed. Documentation `0.3.0` is **not** bumped. Application repos stay `0.1.x`. Ticket **#24** stays cancelled. Ticket **#37** stays **Not Ready**. Do **not** Todo **#37**.
