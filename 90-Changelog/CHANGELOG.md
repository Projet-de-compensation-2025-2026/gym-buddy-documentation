...
- Approved frontend toolchain is **TypeScript 6** (`~6.0.2` on `gym-buddy-ui` `develop`) and **pnpm** with a four-week `minimumReleaseAge` of **40320** minutes (`pnpm-workspace.yaml`; older pnpm `.npmrc` `minimum-release-age=40320`). Corepack pin, committed `pnpm-lock.yaml`, no `latest`, `onlyBuiltDependencies` and/or ignore-scripts, Renovate/Dependabot cooldown ≥ four weeks. Do not invent a `package.json` field for the age floor. Today `gym-buddy-ui` `develop` is **`pnpm@11.22.0`** (ui #4 / `63bebed`; ticket **#23** Done; committed lockfile; `minimumReleaseAge` **40320**) and TypeScript `~6.0.2`. Joaquim cancelled the TypeScript 6→7 migration (ticket #24 cancelled/closed). Reason: `typescript@7.0.0` is 404 on npm; latest 7.0.x Kernel found is **7.0.2** (2026-07-08, Go `tsc`); 7.0.2 installs and standalone typecheck passes; `ng build` fails because Angular 22 `@angular/compiler-cli` **22.1.2** still uses the TS 6 API (`ts.readConfigFile is not a function`; peer `>=6.0 <6.1`). Kernel did not pin `@typescript/native-preview`, `tsgo`, or Angular next. App version stays `0.1.0`. Do **not** claim TypeScript 7 shipped. Do **not** treat 7.0.0 or 7.0.2 as approved or as a next step.
