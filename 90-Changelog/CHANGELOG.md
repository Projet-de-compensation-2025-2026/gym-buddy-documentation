...
- VPS apply **done**. Ticket **#20** is **Done / closed**. Sentinel confirmed the rebuild: VPS Java container on the host is `gym-buddy-service` `develop` **`aea1c56`** (not still `e2ef2aa`; CORS proof used that SHA). Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` on `127.0.0.1:8080` both **200**. API bind `127.0.0.1`; data-plane ports unpublished. A bad loopback `POST /api/v1/auth/register` → **422 `VALIDATION`** (auth routes exist). `replace.sh` skip-pull for local tags is on `develop` ([gym-buddy-service#8](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/8) / `fb1e618`). That is what is true about `replace.sh`. **Not** a GHCR pull, a Release tag, or a successful replace-from-registry. Ticket **#12** stays closed (openapi #4 + ui #3 + service #5).
- Caddy is **proven from the operator network** (Sentinel, from his PC): `GET /api/v1/healthz` → **200**; `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**; `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. Caddy is **not** proven from the GitHub Pages origin. Login-from-Pages is **not** proven. UI `develop` **`7916fa8`** has production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. Service `develop` **`aea1c56`** CORS is **proven from Joaquim’s PC**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. First tag **v0.1.0** pointed at `http://127.0.0.1:8080/api/v1`. Live Pages is **v0.1.1** and embeds the VPS `apiBaseUrl`. Ticket **#31** is **Done / closed** for **only** `apiBaseUrl` + CORS + that live bundle. Do **not** write that signup/login from github.io works. UFW 443 is Joaquim’s IPv6 prefix only (do not publish that prefix). Documentation `0.3.0` is **not** bumped.
...
- `gym-buddy-ui` GitHub Pages **live**. Ticket **#30** is **Done**. First UI tag was **v0.1.0** (localhost `apiBaseUrl`). Current live tag is **v0.1.1**. Bundle `main-4WJYST2C.js` embeds `https://vps-c39cdf03.vps.ovh.net/api/v1` (no `127.0.0.1`) and includes the password eye. Sentinel confirmed https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/ returns **HTTP 200** and serves the Angular app with production `baseHref` `/gym-buddy-ui/`. Root 200 is the acceptance. Direct `/register` returns **HTTP 404** with the SPA index body (`404.html`). That is Pages’ static fallback, **not** a broken app and **not** a working auth route. Ticket **#31** is **Done / closed** (Atlas). UI `develop` **`7916fa8`** has that VPS `apiBaseUrl`. Service `develop` **`aea1c56`** CORS is **proven from Joaquim’s PC**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. Do **not** write that signup/login from github.io works. Login-from-Pages is **not** proven. Blockers: UFW 443 is Joaquim’s IPv6 prefix only (do not publish that prefix); refresh cookie is `SameSite=Lax` and will **not** ride a github.io → VPS credentialed XHR. Login-from-Pages is docs **#37**, board **Not Ready**. Do **not** Todo **#37**. Do **not** start Kernel. Joaquim’s home-browser try is **not** Done. Ticket **#34** stays Done. Ticket **#24** stays cancelled. Approved toolchain stays TypeScript **`~6.0.2`** + **pnpm**.
- Password visibility toggle (eye) is on `gym-buddy-ui` `develop` (`75fbbce` / ui #9 / ticket #34 Done) and in the live **v0.1.1** bundle (`main-4WJYST2C.js`). Ticket **#31** is **Done / closed** for **only** `apiBaseUrl` + CORS + that live bundle. Login-from-Pages is **not** proven. Do **not** write that signup/login from github.io works. Documentation `0.3.0` is **not** bumped.
- CORS is **proven** from Joaquim’s PC on service `develop` **`aea1c56`**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. UI `develop` **`7916fa8`** has production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. Live Pages is **v0.1.1** and embeds that VPS URL. First tag **v0.1.0** pointed at localhost. Ticket **#31** is **Done / closed** for **only** `apiBaseUrl` + CORS + live **v0.1.1**. Login-from-Pages is **not** proven. Do **not** write that signup/login from github.io works. Ticket **#34** stays Done. Ticket **#24** stays cancelled. Documentation `0.3.0` is **not** bumped.
- Login-from-Pages is **not** proven. It is a **stronger** claim than ticket **#31**. Ticket **#31** is **Done / closed** for **only** production `apiBaseUrl` + CORS + live **v0.1.1** (`main-4WJYST2C.js` embeds the VPS URL and the password eye). Do **not** write that signup/login from github.io works. Two named blockers: UFW 443 is Joaquim’s IPv6 prefix only (do not publish that prefix); the refresh cookie is `SameSite=Lax`, so it will **not** ride a github.io → VPS credentialed XHR. Login-from-Pages is docs **#37**, board **Not Ready**. Do **not** Todo **#37**. Do **not** start Kernel. Joaquim can try from his home browser; that is **not** Done. Documentation `0.3.0` is **not** bumped.
- VPS Java container on the host is `gym-buddy-service` `develop` **`aea1c56`** (not still `e2ef2aa`). CORS proof used that SHA. Auth is on that VPS container. Caddy register/login is **proven from the operator network**. Do **not** claim login-from-Pages. Ticket **#31** is **Done / closed**. Ticket **#34** stays Done. Ticket **#24** stays cancelled. Documentation `0.3.0` is **not** bumped.
