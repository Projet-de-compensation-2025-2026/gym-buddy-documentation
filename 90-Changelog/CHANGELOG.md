...
- VPS apply **done**. Ticket **#20** is **Done / closed**. Sentinel confirmed the rebuild: VPS container is `gym-buddy-service` `develop` **`e2ef2aa`** (service #5; auth on develop), **not** a pre-auth snapshot. Loopback `GET /actuator/health/healthz` and `GET /actuator/health/readyz` on `127.0.0.1:8080` both **200**. API bind `127.0.0.1`; data-plane ports unpublished. Empty/invalid `POST /auth/register` → **422 `VALIDATION`** (route live; **not** a successful signup or login). Do **not** write a completed register/login on the VPS. `replace.sh` skip-pull for local tags is on `develop` ([gym-buddy-service#8](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/8) / `fb1e618`). That is what is true about `replace.sh`. **Not** a GHCR pull, a Release tag, or a successful replace-from-registry. Do not claim Caddy is proven. Ticket **#12** stays closed (openapi #4 + ui #3 + service #5).
