...
- Local compose runtime is proven on a laptop (ticket #19 / [gym-buddy-service#6](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/6) / `025a351`): `cp .env.example .env && docker compose up -d --build`; `GET /api/v1/healthz` and `GET /api/v1/readyz` both 200 `{"status":"ok"}`; binds `127.0.0.1` only (`8080`, `5432`, `6379`, `9000`, `9001`). Evidence: `gym-buddy-service` [`docs/local-compose-proof.md`](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/blob/develop/docs/local-compose-proof.md). Not the VPS. Ticket #12 stays open.
- VPS data-plane **files** on `gym-buddy-service` `develop` (ticket #20 / [gym-buddy-service#7](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/7) / `a07e21e`): `deploy/compose.yaml` (PostgreSQL 18.6, Redis, MinIO on private network `gym-buddy-data`, no published ports), `deploy/replace.sh` joins that network and injects `/etc/gym-buddy/vps.env`, `deploy/vps.env.example`, operator runbook [`docs/vps-data-plane.md`](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/blob/develop/docs/vps-data-plane.md). **Apply is not done.** Do not claim VPS `healthz` / `readyz` 200. Ticket #20 stays open.
