...
| `openapi/bundled.yaml` | Ticket **#40** Done: checked-in file. May stay in `gym-buddy-openapi` for CI. It is **not** a consumer input anymore. Dual maintenance can drop. Do **not** claim the file was deleted. |
...
4. **Today** the OpenAPI repo **is** a multi-file `$ref` tree plus a checked-in `bundled.yaml` that may stay for CI. Editors edit the `$ref` tree. Both consumers generate from the tree. `bundled.yaml` is **not** a consumer input anymore. Dual maintenance can drop. Do **not** claim the file was deleted. Do **not** treat the bundle as the editing source.
...
2. Change the `$ref` tree in `gym-buddy-openapi` on a Gitflow `feature/#n-…` branch (`openapi/openapi.yaml` and the files it `$ref`s). Do not hand-edit `bundled.yaml` as source. `bundled.yaml` may stay for CI. It is **not** a consumer input anymore. Dual maintenance can drop.
