...
| | Today | Target |
| --- | --- | --- |
| Document | OpenAPI 3.1 (`info.version` `0.1.0`). **Landed:** `$ref` tree plus a checked-in `openapi/bundled.yaml` ([gym-buddy-openapi#5](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-openapi/pull/5) / ticket **#40** Done; `develop` **`7fa5108`**). Editors edit the `$ref` tree. Both consumers generate from tag **v0.1.0** `$ref` tree (tickets **#47** / **#48** Done). `openapi/bundled.yaml` is **not** a consumer input. It **can be omitted**. It **can be deleted**. Dual maintenance of a second consumer document is no longer required. Do **not** claim the file is already deleted. Atlas will ticket the delete (OpenAPI file + any CI/scripts that still require the bundle) after this wiki is on `develop`. OpenAPI CI `test.sh` still requires the file today. Git tag **v0.1.0** exists (ticket **#46** Done). | Full `/api/v1`. Keep the versioned package / tag. Generators keep reading `openapi/openapi.yaml`. No second consumer document. |
...
This page is still the contract source of truth. The service on `develop` implements those four auth operations (service #5 / `e2ef2aa`; today’s codegen is service #11 / **`3ffdef8`**; ticket **#41** / **`c40f122`** is history). Ticket #12 is closed. Those operations run on the VPS container (`develop` **`aea1c56`**). Caddy register/login is **proven from the operator network**. Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven. Do **not** Todo **#37**. Refresh cookie stays `SameSite=Lax` (unchanged). Ticket **#40** is **Done**. Ticket **#41** is **Done**. Ticket **#42** is **Done**. Ticket **#46** is **Done**. Ticket **#47** is **Done**. Ticket **#48** is **Done**. Do **not** un-Done them. Git tag **v0.1.0** exists. Both consumers pin **v0.1.0** and generate from `openapi/openapi.yaml`.
...
4. **Today** the OpenAPI repo **is** a multi-file `$ref` tree plus a checked-in `bundled.yaml` that **can be omitted**. Editors edit the `$ref` tree. Both consumers generate from the tree. `bundled.yaml` is **not** a consumer input. It **can be deleted**. Dual maintenance of a second consumer document is no longer required. Do **not** claim the file is already deleted. Atlas will ticket the delete (OpenAPI file + any CI/scripts that still require the bundle) after this wiki is on `develop`. Do **not** treat the bundle as the editing source.
...
8. Documentation stays **`0.3.0`**. Application repos stay **`0.1.x`**. Service `pom.xml` stays **0.2.0-SNAPSHOT**. Ticket **#24** stays cancelled. Ticket **#37** stays **Not Ready**. Do **not** claim login-from-Pages. Do **not** Todo **#37**. Ticket **#40** is **Done**. Ticket **#41** is **Done**. Ticket **#42** is **Done**. Ticket **#46** is **Done**. Ticket **#47** is **Done**. Ticket **#48** is **Done**. Do **not** reopen #36 / #38 / #39 / #43 / #44 / #45 / #49 / #50.
...
2. Change the `$ref` tree in `gym-buddy-openapi` on a Gitflow `feature/#n-…` branch (`openapi/openapi.yaml` and the files it `$ref`s). Do not hand-edit `bundled.yaml` as source. `bundled.yaml` is **not** a consumer input. It **can be omitted**. Dual maintenance of a second consumer document is no longer required. Do **not** claim the file is already deleted. Atlas will ticket the delete (OpenAPI file + any CI/scripts that still require the bundle) after this wiki is on `develop`.
