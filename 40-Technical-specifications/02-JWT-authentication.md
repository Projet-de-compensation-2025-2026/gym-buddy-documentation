...
`SameSite=Lax` is enough for same-site operator/Caddy login. It will **not** ride a github.io → VPS credentialed XHR. Ticket **#31** is **Done / closed** for **only** production `apiBaseUrl` + CORS + live **v0.1.1** (`main-4WJYST2C.js`). Login-from-Pages is a **stronger** claim and is **not** proven. Do **not** write that signup/login from github.io works. The other named blocker is UFW 443 (Joaquim’s IPv6 prefix only; do not publish that prefix). Atlas is opening a **Not Ready** ticket for login-from-Pages. Do **not** Todo it. Do **not** start Kernel. Joaquim can try from his home browser; that is **not** Done.
