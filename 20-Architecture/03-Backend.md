...
| Contract | Service implements `GET /api/v1/healthz` and `GET /api/v1/readyz`, plus `POST /api/v1/auth/register`, `/login`, `/refresh`, `/logout` (service #5 / `e2ef2aa`). OpenAPI documents the same four auth paths (openapi #4). Public contract is **not** `/actuator/health`. **Landed:** `generate-sources` fetches `openapi/bundled.yaml` from `gym-buddy-openapi` `develop` / **`7fa5108`** ([gym-buddy-service#10](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/10) / ticket **#41** Done; `develop` **`c40f122`**). Branch `feature/41-openapi-codegen` is gone. `AuthController` implements `AuthApi`; `HealthController` implements `DefaultApi`. Generated sources are **not** committed. Login JSON is `accessToken` only (spec). Handle `minLength` 1. SameSite unchanged. `pom.xml` stays **0.2.0-SNAPSHOT**. Do **not** treat `springdoc` `/v3/api-docs` as the source of truth. | Full `/api/v1`; health stays `healthz` / `readyz`. Depend on the versioned `gym-buddy-openapi` **package / tag**. Generate models + API interfaces from the `$ref` tree (`openapi/openapi.yaml`) at build — **not** `bundled.yaml`, **not** a raw develop SHA GET. Controllers implement generated interfaces. `pom.xml` is the consumer, not a second contract. Do **not** commit generated sources. |
