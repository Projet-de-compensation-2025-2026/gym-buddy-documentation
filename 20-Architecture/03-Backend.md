...
| Data plane | Local compose **proven on a laptop** (PostgreSQL 18.6, Redis, MinIO; [`docs/local-compose-proof.md`](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/blob/develop/docs/local-compose-proof.md)). Flyway **V1** + **V2** (`users` + `profiles`). VPS apply **done** (ticket #20 **Done**): image **`gym-buddy-service:local`**; `healthz` / `readyz` 200 on `127.0.0.1:8080`; ports unpublished. **Not** a GHCR / Release. Kernel is rebuilding from develop for `e2ef2aa` auth | Same local compose; private VPS data plane; full domain schema |
