...
| Contract | Service implements `GET /api/v1/healthz` and `GET /api/v1/readyz`, plus `POST /api/v1/auth/register`, `/login`, `/refresh`, `/logout` (service #5 / `e2ef2aa`). OpenAPI stub documents the same four auth paths (openapi #4). Public contract is **not** `/actuator/health`. **Today** those shapes are handwritten Java DTOs and controllers. **No** `openapi-generator` (or equivalent) at build. Do **not** treat `springdoc` `/v3/api-docs` as the source of truth. | Full `/api/v1`; health stays `healthz` / `readyz`. Generate models + API interfaces from the `gym-buddy-openapi` bundle at build. Controllers implement generated interfaces. `pom.xml` is the consumer, not a second contract. Do **not** commit hand-edited generated sources. |
