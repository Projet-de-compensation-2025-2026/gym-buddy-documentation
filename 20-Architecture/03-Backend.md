...
| Data plane | Local compose **proven on a laptop** (PostgreSQL 18.6, Redis, MinIO; [`docs/local-compose-proof.md`](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/blob/develop/docs/local-compose-proof.md)). Flyway **V1** + **V2** (`users` + `profiles`). VPS apply **done** (ticket **#20** **Done / closed**): container `develop` **`e2ef2aa`** (service #5; not a pre-auth snapshot); loopback `GET /actuator/health/healthz` and `GET /actuator/health/readyz` **200** on `127.0.0.1:8080`; API bind `127.0.0.1`; ports unpublished. Empty/invalid `POST /auth/register` → **422 `VALIDATION`** (route live). `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull / Release / replace-from-registry | Same local compose; private VPS data plane; full domain schema |

Do not write a completed register/login on the VPS. Empty/invalid `POST /auth/register` is **422 `VALIDATION`** only. Do not claim Caddy is proven, or domain tables beyond Flyway V2. Local laptop compose is proven. Service auth is on `develop` (`e2ef2aa`). Ticket #12 is closed / Done. Ticket **#20** is **Done / closed**.
