...
| HTTP client | Handwritten `src/app/api/models.ts` + `auth-api.service.ts`. **No** orval / openapi-typescript (or equivalent) at build. Do **not** vendor `openapi.yaml`. | Generate TS client/types from the `gym-buddy-openapi` repo / tag / bundle (orval or openapi-typescript). Consume that bundle. Do **not** copy the YAML into this repo. |
