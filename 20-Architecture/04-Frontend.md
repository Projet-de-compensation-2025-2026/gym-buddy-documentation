...
**Today (ticket #30 Done):** GitHub Pages hosts the production build at https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/. Sentinel confirmed the root returns **HTTP 200** and serves the Angular app with production `baseHref` `/gym-buddy-ui/`. Root 200 is the acceptance. Direct `/register` returns **HTTP 404** with the SPA index body (`404.html`). That is Pages’ static fallback, **not** a broken app and **not** a working auth route. Ticket **#31** stays **open / In Progress**. UI `develop` **`7916fa8`** has production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. Live Pages is **v0.1.1** and embeds that VPS URL. Service `develop` **`aea1c56`** CORS is **proven from Joaquim’s PC**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. First tag **v0.1.0** pointed at `http://127.0.0.1:8080/api/v1`. Ticket **#31** is **not Done**. Do **not** claim login-from-Pages. Sentinel still has to confirm login-from-Pages.
...
| GitHub Pages (today) | Live tag is **v0.1.1** and embeds `https://vps-c39cdf03.vps.ovh.net/api/v1`. UI `develop` **`7916fa8`** has that VPS `apiBaseUrl`. First tag **v0.1.0** pointed at `http://127.0.0.1:8080/api/v1`. The live static app is https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/ (HTTP **200**; ticket **#30** Done). Ticket **#31** stays **open / In Progress** and is **not Done**. Do **not** claim login-from-Pages. Sentinel still has to confirm login-from-Pages. |
