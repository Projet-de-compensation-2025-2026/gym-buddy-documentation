...
**Today (ticket #30 Done):** GitHub Pages hosts the production build at https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/. Sentinel confirmed the root returns **HTTP 200** and serves the Angular app with production `baseHref` `/gym-buddy-ui/`. Root 200 is the acceptance. Direct `/register` returns **HTTP 404** with the SPA index body (`404.html`). That is Pages’ static fallback, **not** a broken app and **not** a working auth route. Ticket **#31** is **Todo** (production `apiBaseUrl` still `http://127.0.0.1:8080/api/v1`; Kernel implementing). Do **not** claim the Pages app talks to the VPS. Do **not** claim login-from-Pages.
...
| GitHub Pages (today) | Production `apiBaseUrl` is still `http://127.0.0.1:8080/api/v1`. The live static app is https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/ (HTTP **200**; ticket **#30** Done). Ticket **#31** is **Todo** (production `apiBaseUrl` still `http://127.0.0.1:8080/api/v1`; Kernel implementing). Do **not** claim the Pages app talks to the VPS. |
| Live API (operator network) | `https://vps-c39cdf03.vps.ovh.net/api/v1` — VPS container is `develop` **`e2ef2aa`**. Caddy is **proven from the operator network** (Sentinel, from his PC): `GET /api/v1/healthz` → **200**; `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**; `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` also **200**. **Not** a GHCR pull / Release. Caddy is **not** proven from the GitHub Pages origin. Ticket **#31** is the remaining work (production `apiBaseUrl` still loopback). Do **not** claim login-from-Pages. |
