...
Today the app has `/register`, `/login`, and a log-out control that call `POST /api/v1/auth/register`, `/login`, `/logout`. Access JWT stays in memory. Refresh cookie credentials are sent (`path /api/v1/auth`). No friends / feed / events. The service on `develop` implements those endpoints (service #5 / `e2ef2aa`). Ticket #12 is closed. Caddy register/login is **proven from the operator network**. Do **not** claim login-from-Pages. A password visibility toggle (eye) is on `gym-buddy-ui` `develop` (`75fbbce` / ui #9 / ticket #34 Done). Live GitHub Pages remains the first tag **v0.1.0** and does **not** include that control.

**Today (ticket #30 Done):** GitHub Pages hosts the production build at https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/. Sentinel confirmed the root returns **HTTP 200** and serves the Angular app with production `baseHref` `/gym-buddy-ui/`. Root 200 is the acceptance. Direct `/register` returns **HTTP 404** with the SPA index body (`404.html`). That is Pages’ static fallback, **not** a broken app and **not** a working auth route. Live Pages **v0.1.0** still points at `http://127.0.0.1:8080/api/v1`. CORS is **proven on the VPS** from the operator network (Sentinel, Joaquim’s PC): the Pages origin gets ACAO + credentials; a foreign origin gets **403**. Ticket **#31** is **not Done** until a new UI **0.1.x Release** embeds production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. Kernel may Release UI 0.1.x now. Do **not** claim that Release already happened. Do **not** claim login-from-Pages.
...
| GitHub Pages (today) | Live **v0.1.0** still points at `http://127.0.0.1:8080/api/v1`. The live static app is https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/ (HTTP **200**; ticket **#30** Done). Ticket **#31** is **not Done** until a new UI **0.1.x Release** embeds production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. Kernel may Release now. Do **not** claim that Release already happened. Do **not** claim login-from-Pages. |
| Live API (operator network) | `https://vps-c39cdf03.vps.ovh.net/api/v1` — VPS container is `develop` **`e2ef2aa`**. Caddy is **proven from the operator network** (Sentinel, from his PC): `GET /api/v1/healthz` → **200**; `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**; `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` also **200**. CORS is **proven on the VPS** from the operator network: Pages origin gets ACAO + credentials; a foreign origin gets **403**. **Not** a GHCR pull / Release. Caddy is **not** proven from the GitHub Pages origin. Ticket **#31** is **not Done** (live Pages **v0.1.0** still localhost). Do **not** claim login-from-Pages. |
