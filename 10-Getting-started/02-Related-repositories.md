...
- Frontend generates a TypeScript client from a tagged spec, not from whatever a local server emitted today (**today landed:** orval 8.22.0 from `openapi/bundled.yaml` / ticket **#42** Done / `b8da6bf`; **target:** versioned package / tag, `$ref` tree — not landed until **#48**)
...
**Today:** `gym-buddy-openapi` `develop` **`7fa5108`** (openapi #5 / ticket **#40** Done) is a `$ref` tree plus a **checked-in** consumer bundle `openapi/bundled.yaml`. OpenAPI 3.1, `info.version` `0.1.0`. Git tag **v0.1.0** exists (ticket **#46** Done; annotated **`6373a11`** → **`9c7c123`** on `main`; `develop` **`5285b7c`**). Editors edit the `$ref` tree. `bundled.yaml` still exists; do **not** delete it until **#48**. Ticket **#47** is **Done**: `gym-buddy-service` `develop` **`3ffdef8`** (service #11). Branch `feature/47-openapi-package` is gone. Only `develop` + `main` remain. `generate-sources` generates Java models + API interfaces from gym-buddy-openapi tag **v0.1.0** `$ref` tree (`openapi/openapi.yaml`). It no longer fetches `develop` / `openapi/bundled.yaml`. Ticket **#41** stays **Done** as history (`c40f122` / service #10 from that bundle). `AuthController` implements `AuthApi`; `HealthController` implements `DefaultApi`. Generated sources are **not** committed. Login JSON is `accessToken` only (spec). Handle `minLength` 1. SameSite=`Lax` unchanged. `pom.xml` stays **0.2.0-SNAPSHOT**. Ticket **#42** is **Done**: `gym-buddy-ui` `develop` **`b8da6bf`** still generates with **orval 8.22.0** from `openapi/bundled.yaml` until **#48**. **No** `openapi.yaml` (or any YAML copy) in the UI tree. Do **not** vendor `openapi.yaml` into `gym-buddy-ui`. Do **not** claim the service is handwritten. Pinning a **tag/version** has **landed for the service** (ticket **#47** Done). It has **not** landed for the UI (ticket **#48** is **In Progress**; the UI still generates from `openapi/bundled.yaml` until it lands). Canonical rules: [../40-Technical-specifications/08-OpenAPI-contract.md](../40-Technical-specifications/08-OpenAPI-contract.md).

Today the OpenAPI contract **and** the service implement `GET /api/v1/healthz` and `GET /api/v1/readyz`. Those remain the public health paths — [../40-Technical-specifications/01-API-conventions.md](../40-Technical-specifications/01-API-conventions.md). The public contract is not `/actuator/health`. The same six routes / operationIds as before document `POST /api/v1/auth/register`, `/login`, `/refresh`, and `/logout` (openapi #4 / ticket #12; packaging is openapi #5 / ticket **#40**). The UI on `develop` has a basic sign-up page, sign-in page, and log-out control that call register / login / logout (ui #3). The service on `develop` implements those operations ([gym-buddy-service#5](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/5) / `e2ef2aa`): Argon2id, HS256 access JWT, refresh cookie, Redis denylist. Ticket #12 is closed. Ticket **#40** is **Done**. Ticket **#41** is **Done**. Ticket **#42** is **Done**. Ticket **#46** is **Done**. Ticket **#47** is **Done**. Ticket **#48** is **In Progress**. The UI still generates from `openapi/bundled.yaml` until it lands. Caddy register/login is **proven from the operator network**. Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven. Do **not** Todo **#37**.
...
A laptop `compose.yaml` in the backend repo starts PostgreSQL 18, MinIO, Redis, and the API. That boot is **proven** ([`docs/local-compose-proof.md`](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/blob/develop/docs/local-compose-proof.md) on `gym-buddy-service` `develop`). The Angular apps point at `http://localhost:8080/api/v1`. **Today** the service generates from gym-buddy-openapi tag **v0.1.0** `$ref` tree (`openapi/openapi.yaml`; ticket **#47** Done; `develop` **`3ffdef8`**). **Today** the UI still generates from `openapi/bundled.yaml` (ticket **#42** Done; `b8da6bf`) until **#48**. Ticket **#41** (`c40f122` from that bundle) stays **Done** as history. Do **not** delete `bundled.yaml` until **#48**. This is **not** the VPS story.
