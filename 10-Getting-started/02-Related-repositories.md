...
VPS apply **is done** (ticket #20 **Done**). Kernel rebuilt the VPS as develop `e2ef2aa` ([gym-buddy-service#7](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/pull/7) / `a07e21e` files). Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` 200 on `127.0.0.1:8080`. Data-plane ports unpublished. **Not** a GHCR / Release. Empty `POST /api/v1/auth/register` → **422 `VALIDATION`** (route live; not a signup / login proof). Do not claim login on the VPS. Do not claim Caddy is proven.
