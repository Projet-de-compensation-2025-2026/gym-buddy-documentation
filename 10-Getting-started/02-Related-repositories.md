...
VPS apply **is done** (ticket #20 **Done**). Java + PostgreSQL 18.6 / Redis / MinIO are up. Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` 200 on `127.0.0.1:8080`. API bind `127.0.0.1`; data-plane ports unpublished. Running image is **`gym-buddy-service:local`**, **not** a GHCR / Release tag. This is **not** a login proof. Kernel is rebuilding the VPS from develop for `e2ef2aa` auth. Do not claim login on the VPS. Do not claim Caddy is proven.
