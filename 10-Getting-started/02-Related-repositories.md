...
- Frontend generates a TypeScript client from a tagged spec, not from whatever a local server emitted today (**today landed:** orval 8.22.0 from `openapi/bundled.yaml` / ticket **#42** Done / `b8da6bf`; **target:** versioned package / tag, `$ref` tree)
- Backend generates models + API interfaces from that spec at build (**today landed:** `generate-sources` from `openapi/bundled.yaml` / ticket **#41** Done / `c40f122`; **target:** versioned package / tag, `$ref` tree)
...
A laptop `compose.yaml` in the backend repo starts PostgreSQL 18, MinIO, Redis, and the API. That boot is **proven** ([`docs/local-compose-proof.md`](https://github.com/Projet-de-compensation-2025-2026/gym-buddy-service/blob/develop/docs/local-compose-proof.md) on `gym-buddy-service` `develop`). The Angular apps point at `http://localhost:8080/api/v1`. **Today** consumers still generate from `openapi/bundled.yaml` via a raw `develop` / **`7fa5108`** GET (tickets **#41** / **#42** Done). **Target:** depend on the versioned `gym-buddy-openapi` **package**; pin a **tag/version**, not a raw develop SHA GET; generators read `openapi/openapi.yaml`. This is **not** the VPS story.
