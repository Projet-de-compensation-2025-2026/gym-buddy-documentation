...
9. `gym-buddy-openapi` is the HTTP source of truth. Edit the `$ref` tree **before** implementing a new route. **Today** both consumers pin tag **v0.1.0** and generate from `openapi/openapi.yaml` (tickets **#47** / **#48** Done). `openapi/bundled.yaml` is **not** a consumer input. It **can be omitted**. Atlas will ticket the delete after this wiki is on `develop`. Ticket **#41** and ticket **#42** stay **Done** as history.
...
**Locked target:** that same Release writes the SemVer into UI `package.json` and service `pom.xml` (and any matching lock / changelog it already touches). Humans do not hand-edit those versions. New application versions stay **0.1.x** unless the wiki says otherwise. **Today** UI and service Release do **not** bump those files. OpenAPI Release already syncs `package.json` + `info.version` — OpenAPI-only. Service `pom.xml` is **0.2.0-SNAPSHOT** today (the file, not a new 0.2.0 product release). UI `package.json` is **0.1.0**. Live UI is **v0.1.1**.
