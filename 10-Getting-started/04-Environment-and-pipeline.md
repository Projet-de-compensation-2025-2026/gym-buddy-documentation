...
| VM | Apply **done**. Ticket **#20** **Done / closed**. Container is `gym-buddy-service` `develop` **`e2ef2aa`** (service #5; not a pre-auth snapshot). Loopback `GET /actuator/health/healthz` and `GET /actuator/health/readyz` **200** on `127.0.0.1:8080`. API bind `127.0.0.1`; data-plane ports unpublished. Empty/invalid `POST /auth/register` → **422 `VALIDATION`** (route live; **not** a completed register/login). `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull / Release / replace-from-registry. Do not claim Caddy is proven. | Same loopback API + private data plane; GHCR replace when a version is tagged |
...
The next slice is **documentation `0.3.0`** (technical foundation). Local compose runtime is **done** on a laptop (ticket #19 / service #6 / `025a351`). Sign-up / sign-in / log-out is **on `develop`** (openapi #4, ui #3, service #5 / `e2ef2aa`; ticket #12 **closed / Done**). VPS apply is **done** (ticket **#20** **Done / closed**; container `develop` **`e2ef2aa`**). `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). Empty/invalid `POST /auth/register` is **422** only — do **not** write a completed register/login on the VPS. The items below are still open.
