...
| --- | --- | --- |
| `gym-buddy-service` | Java 25 LTS / Spring Boot (`pom.xml` on `develop`). Flyway `V1__baseline.sql` and `V2__users_and_profiles.sql`. Implements `POST /api/v1/auth/register`, `/login`, `/refresh`, `/logout` (service #5 / `e2ef2aa`). **Today** those HTTP shapes are handwritten Java DTOs and controllers. **No** `openapi-generator` (or equivalent) at build. Do **not** treat `springdoc` `/v3/api-docs` as the source of truth. `compose.yaml` and `.env.example` are in the repo. VPS Java container on the host is `develop` **`aea1c56`**. `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull / Release / replace-from-registry. Caddy register/login is **proven from the operator network**. Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven. Do **not** Todo **#37**. | Java 25 LTS / Spring Boot modular monolith. Generate models + API interfaces from the `gym-buddy-openapi` bundle at build. Controllers implement generated interfaces. `pom.xml` is the consumer, not a second contract. |
| `gym-buddy-openapi` | OpenAPI 3.1 (`info.version` `0.1.0`). **`$ref` tree plus checked-in** `openapi/bundled.yaml` (openapi #5 / ticket **#40** Done; `develop` **`7fa5108`**). Same six routes / operationIds as before (`getHealthz`, `getReadyz`, `postAuthRegister`, `postAuthLogin`, `postAuthRefresh`, `postAuthLogout`). Editors edit the `$ref` tree. Consumers generate from **`openapi/bundled.yaml`**. The service on `develop` implements the four auth operations. This repo is the HTTP source of truth. Service and UI still **hand-write** the same shapes (tickets **#41** / **#42**). | Full contract; health stays `GET /api/v1/healthz` and `GET /api/v1/readyz`. Same `$ref` tree + **`openapi/bundled.yaml`**. Service / UI generate from that bundle. |
| `gym-buddy-ui` | Angular 22 (app version `0.1.0`; includes `@angular/compiler-cli` **22.1.2**, peer `>=6.0 <6.1`), **TypeScript `~6.0.2`**, **`packageManager`: `pnpm@11.22.0`** (ui #4 / `63bebed`; ticket **#23** Done; committed `pnpm-lock.yaml`; `minimumReleaseAge` **40320**): `/register`, `/login`, and a log-out control that call `POST /api/v1/auth/register`, `/login`, `/logout` (ui #3). HTTP types and auth client are handwritten (`src/app/api/models.ts` + `auth-api.service.ts`). **No** orval / openapi-typescript (or equivalent) at build. Do **not** vendor `openapi.yaml` into this repo. Access JWT in memory. Refresh cookie credentials sent (`path /api/v1/auth`). No friends / feed / events. Service auth is on `develop`. Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven. Do **not** Todo **#37**. GitHub Pages **live** (ticket **#30** Done; first tag **v0.1.0**): https://projet-de-compensation-2025-2026.github.io/gym-buddy-ui/ is HTTP **200**, production `baseHref` `/gym-buddy-ui/`. Root 200 is the acceptance. Direct `/register` is HTTP **404** with the SPA index body (`404.html`) — Pages fallback, not a broken app and not a working auth route. A password visibility toggle (eye) is on `develop` (`75fbbce` / ui #9 / ticket #34 Done). The live **v0.1.1** bundle (`main-4WJYST2C.js`) includes the password eye. UI `develop` **`7916fa8`** has production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. Live Pages is **v0.1.1** and embeds that VPS URL. Service `develop` **`aea1c56`** CORS is **proven from Joaquim’s PC**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. First tag **v0.1.0** pointed at `http://127.0.0.1:8080/api/v1`. Ticket **#31** is **Done / closed**. Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven. Do **not** Todo **#37**. Stay on TypeScript `~6.0.2` until Angular actually supports 7. Joaquim cancelled the TypeScript 6→7 migration (ticket #24 cancelled/closed). Do **not** claim TypeScript 7 landed. | Angular 22 + **TypeScript 6** (`~6.0.2`) + **pnpm** (Corepack pin, committed `pnpm-lock.yaml`, `minimumReleaseAge` **40320** minutes, `onlyBuiltDependencies` and/or ignore-scripts) member app + back-office. Generate TS client/types from the OpenAPI repo / tag / bundle (orval or openapi-typescript). Do **not** vendor a copy of the YAML. |
...
9. `gym-buddy-openapi` is the HTTP source of truth. Edit the `$ref` tree and keep `openapi/bundled.yaml` current **before** implementing a new route. Consumers generate from that bundle.
...
| Expand the OpenAPI contract past health + the four auth operations (rest of `/api/v1`). Repo **MAY** split `$ref` files; it **MUST** publish a bundle. Codegen in service/UI is **target**, **not** landed. | `gym-buddy-openapi` |
