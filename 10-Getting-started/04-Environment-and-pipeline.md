...
| Caddy (operator network) | **Proven** (Sentinel, from his PC). `GET /api/v1/healthz` → **200**. `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**. `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. CORS is **proven on the VPS** from the operator network: Pages origin gets ACAO + credentials; a foreign origin gets **403**. UFW 443 is the operator IPv6 prefix only (do not publish that prefix; the API is not world-readable). Caddy is **not** proven from the GitHub Pages origin. Login-from-Pages is **not** done. Ticket **#31** is **not Done** until a new UI **0.1.x Release** embeds production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1` (live **v0.1.0** still localhost). Kernel may Release now. Do **not** claim that Release already happened. | Same public entry; CORS proven from the operator network; Pages bundle still needs a new 0.1.x Release with production `apiBaseUrl` (#31) |
...
Caddy reverse-proxies the hostname to `127.0.0.1:8080` and obtains a Let’s Encrypt certificate. Caddy is **proven from the operator network** (Sentinel, from his PC): `GET /api/v1/healthz` → **200**; `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**; `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. CORS is **proven on the VPS** from the operator network (Sentinel, Joaquim’s PC): the Pages origin gets ACAO + credentials; a foreign origin gets **403**. Caddy is **not** proven from the GitHub Pages origin. Login-from-Pages is **not** done. Ticket **#31** is **not Done** until a new UI **0.1.x Release** embeds production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1` (live **v0.1.0** still localhost). Kernel may Release now. Do **not** claim that Release already happened. Do **not** claim login-from-Pages. UFW 443 is still only the operator IPv6 prefix (do not publish that prefix; the API is not world-readable).
...
| Work | Where |
| --- | --- |
| New UI **0.1.x Release** that embeds production `apiBaseUrl` `https://vps-c39cdf03.vps.ovh.net/api/v1`. CORS is **proven** from the operator network (Pages origin ACAO + credentials; foreign origin **403**). Live Pages **v0.1.0** still points at localhost. Ticket **#31** is **not Done**. Kernel may Release now. Do **not** claim that Release already happened. Do **not** claim login-from-Pages | `gym-buddy-ui` |
