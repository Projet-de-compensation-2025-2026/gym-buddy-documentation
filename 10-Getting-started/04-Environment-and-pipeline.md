...
| --- | --- | --- |
| `gym-buddy-service` | Java 25 LTS / Spring Boot (`pom.xml` on `develop`). Flyway `V1__baseline.sql` and `V2__users_and_profiles.sql`. Implements `POST /api/v1/auth/register`, `/login`, `/refresh`, `/logout` (service #5 / `e2ef2aa`). Ticket **#41** is **Done** (`develop` **`c40f122`** / service #10): `generate-sources` / openapi-generator fetches `openapi/bundled.yaml` (`gym-buddy-openapi` `develop` / **`7fa5108`**). `AuthController` implements `AuthApi`; `HealthController` implements `DefaultApi`. Generated sources are **not** committed. Login JSON is `accessToken` only (spec). Handle `minLength` 1. SameSite=`Lax` unchanged. `pom.xml` stays **0.2.0-SNAPSHOT**. Do **not** treat `springdoc` `/v3/api-docs` as the source of truth. Do **not** claim the service is handwritten. `compose.yaml` and `.env.example` are in the repo. VPS Java container on the host is `develop` **`aea1c56`**. `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull / Release / replace-from-registry. Caddy register/login is **proven from the operator network**. Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven. Do **not** Todo **#37**. | Java 25 LTS / Spring Boot modular monolith. Depend on the versioned `gym-buddy-openapi` **package / tag**. Generate models + API interfaces from the `$ref` tree (`openapi/openapi.yaml`) at build — **not** `bundled.yaml`, **not** a raw develop SHA GET. Controllers implement generated interfaces. `pom.xml` is the consumer, not a second contract. |
| `gym-buddy-openapi` | OpenAPI 3.1 (`info.version` `0.1.0`). **`$ref` tree plus checked-in** `openapi/bundled.yaml` (openapi #5 / ticket **#40** Done; `develop` **`7fa5108`**). Same six routes / operationIds as before (`getHealthz`, `getReadyz`, `postAuthRegister`, `postAuthLogin`, `postAuthRefresh`, `postAuthLogout`). Editors edit the `$ref` tree. Consumers still generate from **`openapi/bundled.yaml`** until **#47** / **#48**. Git tag **v0.1.0** exists (ticket **#46** Done; annotated **`6373a11`** → **`9c7c123`** on `main`; `develop` **`5285b7c`**). The service on `develop` implements the four auth operations. This repo is the HTTP source of truth. Ticket **#41** is **Done** (`c40f122`). Ticket **#42** is **Done** (`b8da6bf`). Do **not** claim the service is handwritten. | **Versioned package.** Generators read the `$ref` tree (`openapi/openapi.yaml`), **not** a second checked-in `bundled.yaml`. Already tagged **v0.1.0**. Consumers still must pin that **tag/version**, not a raw develop SHA GET — not landed until **#47** / **#48**. Drop dual maintenance: the tree is the edit format; the package/checkout is how consumers see it. `bundled.yaml` is **no longer** the consumer source of truth. Full contract; health stays `GET /api/v1/healthz` and `GET /api/v1/readyz`. |
...
9. `gym-buddy-openapi` is the HTTP source of truth. Edit the `$ref` tree **before** implementing a new route. **Today** consumers still generate from **`openapi/bundled.yaml`** (tickets **#40** / **#41** / **#42** Done) — keep that checked-in bundle current while that is true. **Target:** depend on the versioned **package / tag**; generators read `openapi/openapi.yaml`. `bundled.yaml` is **no longer** the consumer source of truth.
