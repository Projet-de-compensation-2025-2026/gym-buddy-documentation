...
| Caddy (operator network) | **Proven** (Sentinel, from his PC). `GET /api/v1/healthz` → **200**. `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**. `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. UFW 443 is the operator IPv6 prefix only (do not publish that prefix; the API is not world-readable). Caddy is **not** proven from the GitHub Pages origin. Login-from-Pages is **not** done. Ticket **#31** is the remaining work (production `apiBaseUrl` still `http://127.0.0.1:8080/api/v1`). | Same public entry; Pages origin still needs production `apiBaseUrl` / CORS (#31) |
...
| Work | Where |
| --- | --- |
| Point the Pages app at the VPS API and allow that origin (`apiBaseUrl` / CORS). Ticket **#31** is **Todo** (production `apiBaseUrl` still `http://127.0.0.1:8080/api/v1`; Kernel implementing). Do **not** claim login-from-Pages | `gym-buddy-ui` + `gym-buddy-service` |
