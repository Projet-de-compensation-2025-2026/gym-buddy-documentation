...
| `gym-buddy-openapi` | OpenAPI 3.1 (`info.version` `0.1.0`). **`$ref` tree plus checked-in** `openapi/bundled.yaml` (openapi #5 / ticket **#40** Done; `develop` **`7fa5108`**). Same six routes / operationIds as before (`getHealthz`, `getReadyz`, `postAuthRegister`, `postAuthLogin`, `postAuthRefresh`, `postAuthLogout`). Editors edit the `$ref` tree. Consumers generate from **`openapi/bundled.yaml`**. Git tag **v0.1.0** exists (ticket **#46** Done; annotated **`6373a11`** → **`9c7c123`** on `main`; `develop` **`5285b7c`**). The service on `develop` implements the four auth operations. This repo is the HTTP source of truth. Ticket **#41** is **Done** (`c40f122`). Ticket **#42** is **Done** (`b8da6bf`). Do **not** claim the service is handwritten. | **Versioned package.** Generators read the `$ref` tree (`openapi/openapi.yaml`), **not** a second checked-in `bundled.yaml`. Must be **tagged**. Consumers pin a **tag/version**, not a raw develop SHA GET. Drop dual maintenance: the tree is the edit format; the package/checkout is how consumers see it. `bundled.yaml` is **no longer** the consumer source of truth. Full contract; health stays `GET /api/v1/healthz` and `GET /api/v1/readyz`. |
