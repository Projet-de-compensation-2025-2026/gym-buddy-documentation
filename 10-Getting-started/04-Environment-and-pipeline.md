...
| Caddy (operator network) | **Proven** (Sentinel, from his PC). `GET /api/v1/healthz` → **200**. `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**. `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. CORS is **proven from Joaquim’s PC** on **aea1c56**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. Ticket **#31** is **Done / closed** (apiBaseUrl + CORS + live v0.1.1 verified). Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven (UFW 443 IPv6-only; refresh cookie SameSite=Lax). VPS container is **aea1c56**. Password eye is on live **v0.1.1**. Ticket **#34** is **Done**. First tag **v0.1.0** pointed at localhost — that is history, not today. Do **not** Todo **#37**. Joaquim’s Pages login is operator-home only. | Same public entry; **#31** Done; **#37** Not Ready / not proven |
