...
| `gym-buddy-openapi` | OpenAPI 3.1.0 stub (`info.version` `0.1.0`): `GET /healthz` and `GET /readyz` under `/api/v1`, plus `POST /api/v1/auth/register`, `/login`, `/refresh`, `/logout` (openapi #4 / ticket #12). The service on `develop` implements those four operations. This repo is the HTTP source of truth. Service and UI still **hand-write** the same shapes. | Full contract; health stays `GET /api/v1/healthz` and `GET /api/v1/readyz`. **MAY** split into multi-file `$ref` (entities / requests / responses / paths). **MUST** publish a single bundled document consumers generate from. |
