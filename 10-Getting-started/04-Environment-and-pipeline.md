...
| Caddy (operator network) | **Proven** (Sentinel, from his PC). `GET /api/v1/healthz` → **200**. `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**. `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. CORS is **proven from Joaquim’s PC** on **aea1c56**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. Ticket **#31** is **Done / closed** (apiBaseUrl + CORS + live v0.1.1 verified). Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven (UFW 443 IPv6-only; refresh cookie SameSite=Lax). VPS container is **aea1c56**. Password eye is on live **v0.1.1**. Ticket **#34** is **Done**. First tag **v0.1.0** pointed at localhost — that is history, not today. Do **not** Todo **#37**. Joaquim’s home-browser login is an operator-IPv6 try, not Done. | Same public entry; **#31** Done; **#37** Not Ready / not proven |
...
Caddy reverse-proxies the hostname to `127.0.0.1:8080` and obtains a Let’s Encrypt certificate. Caddy is **proven from the operator network** (Sentinel, from his PC): `GET /api/v1/healthz` → **200**; `POST /api/v1/auth/register` (email + handle + password + displayName) → **201**; `POST /api/v1/auth/login` → **200** + access JWT. The API is not the bug. Service `develop` **`aea1c56`** CORS is **proven from Joaquim’s PC**: Pages origin ACAO **200** + credentials; foreign/evil origin **403**. Caddy is **not** proven from the GitHub Pages origin. Ticket **#31** is **Done / closed** (apiBaseUrl + CORS + live v0.1.1 verified). Login-from-Pages is ticket **#37**, **Not Ready**, **not** proven (UFW 443 IPv6-only; refresh cookie SameSite=Lax). VPS container is **aea1c56**. Password eye is on live **v0.1.1**. Ticket **#34** is **Done**. Live Pages is **v0.1.1** and embeds `https://vps-c39cdf03.vps.ovh.net/api/v1`. First tag **v0.1.0** pointed at localhost — that is history, not today. Do **not** Todo **#37**. Joaquim’s home-browser login is an operator-IPv6 try, not Done.
