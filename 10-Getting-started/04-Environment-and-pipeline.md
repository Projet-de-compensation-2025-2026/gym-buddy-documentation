...
| VM | Apply **done** (ticket #20 **Done**). Image **`gym-buddy-service:local`** on `127.0.0.1:8080`. Java + PostgreSQL 18.6 / Redis / MinIO up. `GET /api/v1/healthz` and `GET /api/v1/readyz` 200 on loopback. API bind `127.0.0.1`; data-plane ports unpublished. **Not** a GHCR / Release. **Not** a login proof. Kernel is rebuilding from develop for `e2ef2aa` auth. Do not claim Caddy is proven. | Same loopback API + private data plane; GHCR replace when a version is tagged |
...
That is still **`docker run`** for the API, not `docker compose up -d` of the API. The data-plane file is `deploy/compose.yaml`. **Today’s VPS image is `gym-buddy-service:local`.** That is **not** a GHCR / Release. Kernel is rebuilding from develop for `e2ef2aa` auth.
...
| `deploy/replace.sh` | `docker run` of the API. Joins `gym-buddy-data`. Injects VPS env. `DEPLOY_BIND` default `127.0.0.1`; `0.0.0.0` refused. Today’s image is **`gym-buddy-service:local`**, not a GHCR / Release tag. |
| `docs/vps-data-plane.md` | Operator runbook |

**Apply is done.** Java + PostgreSQL 18.6 / Redis / MinIO are up on the OVH VPS (Sentinel / Orbit):

- `GET /api/v1/healthz` 200 and `GET /api/v1/readyz` 200 on `127.0.0.1:8080`
- Java API bind `127.0.0.1`
- Data-plane ports `5432` / `6379` / `9000` / `9001` unpublished
- Running image **`gym-buddy-service:local`** (not a GHCR / Release tag)

This is **not** a login proof. Kernel is rebuilding the VPS from develop for `e2ef2aa` auth. Do **not** claim login is on the VPS. Caddy was not reachable from Sentinel; do **not** claim Caddy is proven.
...
The next slice is **documentation `0.3.0`** (technical foundation). Local compose runtime is **done** on a laptop (ticket #19 / service #6 / `025a351`). Sign-up / sign-in / log-out is **on `develop`** (openapi #4, ui #3, service #5 / `e2ef2aa`; ticket #12 **closed / Done**). VPS apply is **done** on loopback (ticket #20 **Done**; image `gym-buddy-service:local`). Kernel is rebuilding from develop for `e2ef2aa` auth. The items below are still open.
