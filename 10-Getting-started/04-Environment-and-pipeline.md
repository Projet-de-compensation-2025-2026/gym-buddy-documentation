...
| VM | Apply **done**. Ticket **#20** **Done / closed**. Today’s VPS container is **aea1c56**. Loopback `GET /api/v1/healthz` and `GET /api/v1/readyz` **200** on `127.0.0.1:8080`. API bind `127.0.0.1`; data-plane ports unpublished. A bad loopback `POST /api/v1/auth/register` → **422 `VALIDATION`** (auth routes exist). `replace.sh` skip-pull for local tags is on `develop` (service #8 / `fb1e618`). **Not** a GHCR pull / Release / replace-from-registry. | Same loopback API + private data plane; GHCR replace when a version is tagged |
...
That is still **`docker run`** for the API, not `docker compose up -d` of the API. The data-plane file is `deploy/compose.yaml`. Today’s VPS container is **aea1c56**. That is **not** a GHCR pull, a Release tag, or a successful replace-from-registry.
...
| `deploy/replace.sh` | `docker run` of the API. Joins `gym-buddy-data`. Injects VPS env. `DEPLOY_BIND` default `127.0.0.1`; `0.0.0.0` refused. Skip-pull for local tags is on `develop` (service #8 / `fb1e618`). Today’s VPS container is **aea1c56**. **Not** a GHCR pull / Release / replace-from-registry. |
| `docs/vps-data-plane.md` | Operator runbook |

**Apply is done.** Ticket **#20** is **Done / closed**. Sentinel confirmed the VPS rebuild:

- Today’s VPS container is **aea1c56**. Service #5 / `e2ef2aa` is the auth-landing SHA (history of when auth landed), not today’s container.
...
API container `gym-buddy-service` (`DEPLOY_CONTAINER_NAME`; `docker run`, not compose of the API; bound to `127.0.0.1:8080`; today **aea1c56**):
